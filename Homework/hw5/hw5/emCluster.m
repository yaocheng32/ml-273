function [z,mu,R,ll] = emCluster(X,K,cInit,nIter,tol)
...
while (~done) 
   ...
   for c=1:K,
     ...
     if (doPlot && D==2) fig(1); hold on; plotGauss2D(mu(c,:),Sig(:,:,c),'k-','linewidth',2); end;
   end;
   alpha = alpha./N;
   if (doPlot) llplot(iter)=ll; fig(2); plot(1:iter,llplot(1:iter),'b-'); end;
   %pause(.1);
